...
Data :: ~Data(void)
{
  ...
  for(short index = 0; index < this -> packets; ++index)
  {
    ...
    testing_ds[index] = nullptr;
  }

  // Deallocate the dataset.
  delete[] testing_ds; testing_ds = nullptr;
}
...
void Data :: createTrainingData(void)
{
  ...
  for(index = 0; index < this -> packets; ++index)
  {
    ...
    // If the first input is 0, set the target
    // output to 0. Otherwise set the target to 1.
    testing_ds[index][num_inputs] = (fst == 0)? 0:1;
  }
